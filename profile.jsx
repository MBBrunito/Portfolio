export const skills = [
   {
      skill: "Programación",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697208309/skills/Programacion_w3q5dt.png",
      description:
         "Habilidad para escribir código y crear algoritmos para resolver problemas de software.",
   },
   {
      skill: "Lógica",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697208309/skills/Logica_vhifoq.png",
      description:
         "Capacidad para razonar y tomar decisiones basadas en principios lógicos.",
   },
   {
      skill: "Javascript",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726038/skills/JS_cznnif.png",
      description:
         "Lenguaje de programación ampliamente utilizado para el desarrollo web.",
   },
   {
      skill: "NodeJs",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728076/skills/NodeJs_u9nyul.png",
      description: "Entorno de ejecución de JavaScript del lado del servidor.",
   },
   {
      skill: "JQuery",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728062/skills/JQuery_wich4z.png",
      description:
         "Biblioteca de JavaScript diseñada para simplificar la manipulación de documentos HTML.",
   },
   {
      skill: "React",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726038/skills/React_r5xw2z.png",
      description:
         "Biblioteca de JavaScript para construir interfaces de usuario interactivas.",
   },
   {
      skill: "Redux",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726039/skills/Redux_iuvliz.png",
      description:
         "Biblioteca de gestión del estado para aplicaciones JavaScript.",
   },
   {
      skill: "NextJS",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726038/skills/NextJS_gedlt5.png",
      description:
         "Framework de React que permite renderizado del lado del servidor y construcción de aplicaciones web rápidas y eficientes.",
   },
   {
      skill: "NestJS",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726038/skills/NestJS_xpw6nz.png",
      description:
         "Framework de Node.js que facilita la creación de servidores escalables y mantenibles usando TypeScript.",
   },
   {
      skill: "Webpack",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728090/skills/WebPack_unmbix.png",
      description:
         "Herramienta de construcción de módulos estáticos para JavaScript, HTML y CSS.",
   },
   {
      skill: "Jest",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728061/skills/Jest_x9rdgo.png",
      description:
         "Framework de prueba de JavaScript con un enfoque en la simplicidad y el soporte completo de proyectos de gran escala.",
   },
   {
      skill: "SQL",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726038/skills/SQL_d1sljk.png",
      description:
         "Lenguaje de consulta estructurado utilizado para gestionar y manipular bases de datos relacionales.",
   },
   {
      skill: "PostgreSQL",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726039/skills/PostgreSQL_j13y9l.png",
      description:
         "Sistema de gestión de bases de datos relacional de código abierto y altamente potente.",
   },
   {
      skill: "Insomnia",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728061/skills/Insomnia_wfsmd1.png",
      description:
         "Herramienta de prueba de API para diseñar, crear y gestionar API de forma eficiente.",
   },
   {
      skill: "Sequelize",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728090/skills/Sequelize_dtlwmv.png",
      description:
         "ORM de Node.js para bases de datos SQL y PostgreSQL, que ofrece una interfaz sencilla y potente para interactuar con la base de datos.",
   },
   {
      skill: "MySQL",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728075/skills/MySQL_wvnm8u.png",
      description:
         "Sistema de gestión de bases de datos relacional de código abierto ampliamente utilizado para aplicaciones web.",
   },
   {
      skill: "Prisma",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726039/skills/Prisma_cyk4g5.png",
      description:
         "ORM moderno y tipo seguro para bases de datos SQL, diseñado para facilitar la escritura de consultas seguras y mantenibles en TypeScript y JavaScript.",
   },
   {
      skill: "HTML5",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728046/skills/HTML5_brzeey.png",
      description:
         "La última versión del lenguaje de marcado estándar utilizado para crear páginas web y aplicaciones web.",
   },
   {
      skill: "CSS3",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696726038/skills/CSS3_vgt6ic.png",
      description:
         "La última versión del lenguaje de estilo utilizado para definir la presentación de documentos HTML en páginas web.",
   },
   {
      skill: "Less",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728075/skills/Less_qjxuzw.png",
      description:
         "Un preprocesador CSS que permite escribir estilos de manera más eficiente, organizada y fácil de mantener.",
   },
   {
      skill: "BootStrap",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697208309/skills/Bootstrap_qyglc3.png",
      description:
         "Un popular framework de diseño frontend que facilita la creación de interfaces de usuario atractivas y responsivas.",
   },
   {
      skill: "Tailwind",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697208309/skills/Tailwind_jo9l93.png",
      description:
         "Un framework de diseño CSS altamente personalizable que facilita la creación de interfaces de usuario modernas y eficientes.",
   },
   {
      skill: "Git",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728046/skills/Git_bn4s2i.png",
      description:
         "Sistema de control de versiones distribuido utilizado para rastrear cambios en el código fuente durante el desarrollo de software.",
   },
   {
      skill: "GitHub",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728046/skills/GitHub_vbbt8j.png",
      description:
         "Una plataforma de alojamiento de código fuente basada en Git que ofrece control de versiones y colaboración en proyectos de software.",
   },
   {
      skill: "VSCode",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696798510/skills/VSCode_egnfgt.png",
      description:
         "Un editor de código fuente altamente configurable y extensible desarrollado por Microsoft para programadores y desarrolladores web.",
   },
];
export const experiencies = [
   {
      title: "Full Stack Prep Course Assistant / Henry",
      date: "AGO.2023 – SET.2023",
      description: "Experiencia académica. Henry Bootcamp",
      items: [
         "Asistir a estudiantes en la preparación para ingresar al Bootcamp.",
         "Guiar a los alumnos en la descarga e instalación de las herramientas para el cursado.",
         "Exponer en vivo a los postulantes la resolución de ejercicios para su práctica.",
      ],
   },
   {
      title: "Full Stack Teaching Assistant / Henry",
      date: "ABR.2023 – MAY.2023",
      description: "Experiencia académica. Henry Bootcamp",
      items: [
         "Coordinar un grupo de estudiantes para lograr la integración al equipo de estudio.",
         "Asistir a la resolución de ejercicios y promover la colaboración grupal (Pair Programming).",
         "Proponer ideas para la mejora de los procesos del Bootcamp.",
      ],
   },
];
export const projects = [
   {
      name: "Proyecto ADIP WEB",
      description:
         "Desarrollamos y entregamos exitosamente un sistema integral para el Club ADIP, que incluye una plataforma de administración con funciones de control de usuarios, gestión de noticias y categorías, y un sistema de pagos. Esta solución mejoró la eficiencia de la organización al facilitar la gestión de contenidos, el acceso de usuarios y la recopilación de pagos, contribuyendo así a su misión y objetivos.",
      date: "08/2023 - 09/2023",
      image: "https://res.cloudinary.com/daoavxvau/image/upload/v1697208907/Proyectos/AdipWeb_tz1i9n.jpg",
   },
];
export const contact = [
   {
      red: "LinkedIn",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697208981/Redes%20Sociales/LinkedIn_n4x9si.png",
      contact: "https://www.linkedin.com/in/mbbrunito/",
   },
   // {
   //    red: "Gmail",
   //    logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697208981/Redes%20Sociales/gMail_ndgy3f.png",
   //    contact: "MarcosBrunoDev@gmail.com",
   // },
   {
      red: "GitHub",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1696728046/skills/GitHub_vbbt8j.png",
      contact: "https://github.com/MBBrunito",
   },
   {
      red: "Whatsapp",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697315784/Redes%20Sociales/Whatsapp_r7i3xz.png",
      contact: "https://wa.me/543883312242",
   },
   {
      red: "Telegram",
      logo: "https://res.cloudinary.com/daoavxvau/image/upload/v1697315784/Redes%20Sociales/Telegram_dupxg4.png",
      contact: "https://t.me/MarcosBrunoDev",
   },
];
